...
global $db, $uid, $how_many, $from, $campana_id, $contacto_id, $llamada_id, $submit, $status,
...
$r = $db->sql_query($sql) or die("Error al obtener el ciclo de venta de esta concesionaria.<br>$sql");
$numrows = $db->sql_numrows($r);
if ($numrows < 1)
{
	die("<html><head><script>alert('Está intentando dar seguimiento a un prospecto que no está asignado a esta concesionaria. Campaña: $campana_id. Concesionaria: $gid. $numrows');window.close();</script></head></html>");
}
...
// 	$js_extra = "alert('Agregado a la campaña \'$next_campana\'');";//se pasa al siguiente ciclo
	//logear la aktividad
...
$sql = "SELECT
                    ...
                    tel_movil, tel_otro,
                    nota, email, origen_id,

                    tel_casa_2,
                      tel_oficina_2,
                      tel_movil_2,
                      horario_preferido_casa,
                      horario_preferido_oficina,
                      horario_preferido_movil,
                      horario_preferido_casa_2,
                      horario_preferido_oficina_2,
                      horario_preferido_movil_2,codigo_campana,fecha_autorizado,fecha_firmado
...
//        if ($tel_casa)
$horario_preferido_casa = "<table><tr>
...
if ($fecha_cita)
{
    ...
    $select_hora = "<select name=\"hora_cita\" id=\"hora_cita\" value=\"$hora_cita\">";
    for ($h = 0; $h < 24; $h++)
    {
    	$h2 = sprintf("%02d", $h);
    	if ($h == $hora_cita)
    ...
    $select_hora .= "</select>";
    $select_minuto = "<select name=\"minuto_cita\" id=\"minuto_cita\" value=\"$minuto_cita\">";
    for ($m = 0; $m < 60; $m += 5)
    {
    	$m2 = sprintf("%02d", $m);
    	if ($m == $minuto_cita)
...
$catalogo .= "<table width=\"100%\"><th>Catálogo</th></table>";
$cat_productos = array(
                  "Gol Sedan" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/Gol2/Gol_Sedan.html",
                  "Eos" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/Eos.html",
                  "Clasico" => "http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/Clasico2.html",
                  "Nuevo Golf Sportwagen" => "http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/Golf_sportwagen.html",
                  "CrossFox" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/Fox.html",
                  "Nuevo Jetta" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/jetta.html",
                  "Beetle" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/beetle_mx.html",
                  "Beetle Black" => "http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/beetle_mx/Beetle_Black.html",
                  ...
                  "Touareg" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/touareg.html",
					'Passat CC'=> 'http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/PassatCC.html',
					'Tiguan'=> 'http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/Tiguan.html',
					'Routan'=> 'http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/Routan.html',
					'Gol'=> 'http://mx.volkswagen.com/vwcms/master_public/virtualmaster/es_mx/models/Gol2.html'
...
//botones extra
$catalogo .= "<table width=\"100%\"><th>Accesos R&aacute;pidos</th></table>";
$catalogo .= "<input class=boton type=button style=\"width:120px;\" value=\"Promociones\" onclick=\"window.open('http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/Arg/Promociones.html','promociones');\"><br>";
$catalogo .= "<input class=boton type=button style=\"width:120px;\" value=\"Compara\" onclick=\"window.open('http://mx.volkswagen.clientsites.carspecs.jato.com/mx.volkswagen/browser.asp?screen=select&category=select_model&make=Volkswagen&translated_make=Volkswagen','promociones');\"><br>";
$catalogo .= "<input class=boton type=button style=\"width:120px;\" value=\"Configura y cotiza\" onclick=\"window.open('http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/Arg/used_cars.html','promociones');\"><br>";
...
//extra objeciones
$extra_objeciones = "<table width=\"100%\"><th>Di&aacute;logos</th></table>";
...
$fase_id = substr($campana_id,(strlen($campana_id)-2 ),2);
if($gid)
    $fase_id=str_replace($gid,'',$campana_id);
...
//$select_actividades .= $script_act."<br><br><br><br><input type='checkbox' name='f_decisiva' id='f_decisiva' value='1'>Fase Decisiva del CRM Contactos";
...
/*
$sql="SELECT contacto_id FROM crm_contactos_finalizados WHERE contacto_id = '$contacto_id'";
$result = $db->sql_query($sql) or die("Error en llamadas finalizadas ".print_r($db->sql_error()));
$id_llamada = $db->sql_numrows($result);
if(!($id_llamada > 0))
{
  $opciones_botones_venta = "<input type=\"button\" style=\"width:180px;\" value=\"Reportar venta\" onclick=\"window.open('index.php?_module=$_module&_op=llamada_venta&contacto_id=$contacto_id&campana_id=$campana_id', 'venta','location=no,resizable=yes,scrollbars=yes,navigation=no,titlebar=no,directories=no,width=600,height=250,left=0,top=0,alwaysraised=yes');\"><br><br>";
}*/
...
if($num_unidades == 0)
{
    $opciones_botones_venta = "El prospecto no tiene asociado ningun modelo;";
}
else
{
    $num_ventas=0;
...
$opciones_botones_venta.= "<b>$num_unidades modelo$s asignado$s<br>$num_ventas venta$ss registrada$ss<b><br>";

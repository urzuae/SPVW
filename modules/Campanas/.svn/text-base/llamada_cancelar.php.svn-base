...
if ($submit)
{
  ...
  //actualizar el status como finalizado
	...
	$db->sql_query($sql) or die($sql);
    $sql = "delete from reporte_contactos_asignados WHERE contacto_id='$contacto_id'";
	$db->sql_query($sql) or die($sql);
...
$result2 = $db->sql_query("SELECT motivo_id, motivo FROM crm_prospectos_cancelaciones_motivos WHERE 1 order by motivo") or die("Error".print_r($db->sql_error()));

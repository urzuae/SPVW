...
// if($db->sql_numrows($db->sql_query($sql)) < 1) die("<html><head><script>alert('No está autorizado para acceder aquí.');window.close();</script></head></html>");
...
if($db->sql_numrows($result) > 0)
{
    ...
    if(count($tmp_m) > 0)
    {
        ...
        for ($i=0;$i < count($array_titulos); $i++)
        {
            ...
            $buffer_modelo.="<tr class=\"row".($class_row++%2?"2":"1")."\"><td width='$tmp_width%'><b>".$array_titulos[$i]."</b></td>";
...
$cat_productos = array("Pointer" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/POinter2.html",
                  ...
                  "SportVan" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/multivan.html",

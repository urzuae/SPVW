...
$email='';
$sql="SELECT email FROM crm_contactos WHERE contacto_id=".$contacto_id." limit 1;";
...
$buffer="El contacto no tiene veh&iacute;culos asociados";

$buffer="<table width='100%' align='center' border='0'>";
if($email=='')
{
    $buffer.="
            <tr><td>Por favor antes de teclear los datos del chasis y precio, teclee el correo electrónico del contacto</td></tr>
            <tr><td>Correo electr&oacute;nico:&nbsp;&nbsp;
                <input type='text' name='email' id='email' size='45' value=''>
                <input type='button' name='guarda_email' id='guarda_email' value='Guardar Email' onClick=\"actualiza_email('$contacto_id','$campana_id','$llamada_id','$nopendientes');\">
            </td></tr>";
}
else
{
    $buffer.="
            <tr><td>Correo electr&oacute;nico del contacto:&nbsp;&nbsp;$email</td></tr>";

}
$buffer.="</table>";
// Consulta la tabla para sacar los modelos seleccionados por el prospecto
$sql="SELECT modelo,version,ano,paquete,tipo_pintura,color_exterior,color_interior,modelo_id,version_id,transmision_id,timestamp
...
if($db->sql_numrows($res) > 0)
{
        ...
        <th width='12%'>Versi&oacute;n</th>
        <th width='12%'>Transmisi&oacute;n</th>
        <th width=' 5%'>A&ntilde;o</th>
        <th width=' 9%'>Color Exterior</th>
        <th width=' 9%'>Color Interior</th>
        <th width=' 9%'>Tipo de pintura</th>
        <th width=' 9%'>No Chasis</th>
        ...
        while(list($modelo,$version,$ano,$paquete,$tipo_pintura,$color_exterior,$color_interior,$modelo_id,$version_id,$transmision_id,$timestamp) = $db->sql_fetchrow($res))
        {
            ...
            if(count($venta_registrada)>0)
                $read='Readonly';
            if( (count($venta_registrada) == 0) && (!$email))
                $read='Readonly';
            $buffer.="<tr>

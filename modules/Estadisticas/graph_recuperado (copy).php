...
while(list($empresa_id, $nombre) = $db->sql_fetchrow($result))
{
  ...
  while(list($campana_id) = $db->sql_fetchrow($result2))
  {
    ...
    $sql = "SELECT SUM(o.saldo) FROM crm_campanas_llamadas AS c, crm_contactos AS o WHERE c.contacto_id=o.contacto_id AND (c.status_id='-3' OR c.status_id='-4') AND campana_id='$campana_id'$where";
  ...
  array_push($datax, $nombre . $percent);
...
$graph->img->SetMargin(60,20,30,20 + $nombre_mas_largo*6+25);

...
global $db, $how_many, $from, $campana_id, $nombre, $apellido_paterno, $apellido_materno, 
        $submit, $status_id, $ciclo_de_venta_id, $uid, $orderby, $contacto_id;
...
if (!$contacto_id)
{
	die("<h1>Ingrese a esta opción solo después de crear un prospecto</h1>");
}
...
{
	die("<h1>No está autorizado a realizar esta operación</h1>");
}
//reasignar
global $asignar_a;
if ($asignar_a) //si se van a reasignar 
{
	...
	$sql = "INSERT INTO crm_contactos_asignacion_log (contacto_id,uid,from_uid,to_uid)VALUES('$contacto_id','-1','0','$asignar_a')";
	$db->sql_query($sql) or die("Error al leer".print_r($db->sql_error()));
	
	die("<html><head><script>alert('Prospecto asignado');location.href='index.php?_module=$_module&_op=contacto';</script></head></html>");//regresar
}
...
if ($db->sql_numrows($result) < 1)
{
	die("<html><head><script>alert('Esta concesionaria no tiene vendedores, no se pudo asignar.');location.href='index.php?_module=$_module&_op=contacto';</script></head></html>");//regresar
}
 ...
 */ 
	$tabla_campanas .= "<td><a href=\"index.php?_module=$_module&_op=$_op&contacto_id=$contacto_id&asignar_a=$c_uid\">Asignar</a></td>";

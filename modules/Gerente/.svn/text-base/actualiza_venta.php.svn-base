...
global $db,$contacto_id,$modelo_id,$version_id,$transmision_id,$timestamp,$chasis,$fecha_ini,$fecha_fin,$fuente_id,$tipo_user,$vendedor_id,$uid;
...
if($tipo_user == 2)
    $url_regreso="index.php?_module=Gerente&_op=monitoreo_tasa_de_conversion_vendedor&fecha_ini=$fecha_ini&fecha_fin=$fecha_fin&fuente_id=$fuente_id&uid=$uid&vendedor_id=$vendedor_id&submit=submit";
...
if( $num > 0)
{
    ...
    $buffer="<input type='hidden' name='contacto_id' id='contacto_id' value='".$c."'>
            ...
            $buffer.="<tr><td colspan='2' align='center'>
                <input type='button' name='".$boton."' id='".$boton."' value='Registra' onclick=\"actualiza_venta('$unidades[$modelo_id]','$contacto_id','$modelo_id','$version_id','$transmision_id','$timestamp','$tmp_name_chasis','$tmp_name_precio','$uid','$boton','$chasis','$precio');\">
                &nbsp;&nbsp;
                <input type='button' name='cerrar' id='cerrar' value='Cerrar' onclick=\"self.close();window.opener.location.href='".$url_regreso."'\"Onclick=\"cerrar_ventana('$fecha_ini','$fecha_fin','$fuente_id');\">
              </td></tr><table>

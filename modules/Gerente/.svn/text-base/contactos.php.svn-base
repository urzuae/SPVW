...
if ($super > 6) {
    ...
    if ($asignar_a && $submit_seleccionar)
    {
        //buscar a que campaï¿½a lo meteremos
        $sql = "SELECT c.campana_id FROM crm_campanas_groups AS g, crm_campanas AS c WHERE c.campana_id=g.campana_id AND g.gid='$gid' ORDER BY c.campana_id  LIMIT 1"; 
        $result = $db->sql_query($sql) or die("Error al leer" . $sql);
        list($campana_id) = $db->sql_fetchrow($result);

        $sql = "SELECT c.contacto_id FROM crm_contactos AS c  WHERE gid='$gid' order by contacto_id;";
        $result = $db->sql_query($sql) or die("Error al leer" . $sql);
        if ($db->sql_numrows($result) > 0)
        {
            while (list($contacto_id) = $db->sql_fetchrow($result))
            { //revisar si lo mandaron en el post ( => on)
                ...
                if (array_key_exists($tmp, $_POST))
                {
                    ...
                    $res1 = $db->sql_query($sql) or die("Error al leer" .$sql);
                    if ($db->sql_numrows($res1) > 0)
                        list($campana_id) = $db->sql_fetchrow($res1);

                     //buscar quien lo tiene
                      $sql = "SELECT uid FROM crm_contactos WHERE contacto_id='$contacto_id'";
            ...
            if ($no_asignados)
            $extra_js .= "alert('$no_asignados prospectos no fueron reasignados debido a que ya estaban asignados al mismo vendedor.');";
...
//   if ($no_asignados) {$no_asignados_checked = "CHECKED"; $where .= "AND uid=0 ";}
    if ($contacto_id)
        $where .= "AND c.contacto_id LIKE '%$contacto_id%' ";
    if ($nombre)
    ...
    if ($telefono)
        $where .= "AND (c.tel_casa LIKE '%$telefono%' OR c.tel_oficina LIKE '%$telefono%' "
                . "OR c.tel_movil LIKE '%$telefono%' OR c.tel_otro LIKE '%$telefono%')";
    if ($buscar_asignado)
        $where .= " AND c.uid='$buscar_asignado'";
    if ($where == "")
        $where .= "AND c.uid=0 "; //los no asignados
        //ahora si hacemos el query limitado
    $sql = "SELECT c.contacto_id, c.origen_id, c.nombre, c.apellido_paterno, c.apellido_materno, c.tel_casa, c.tel_oficina, c.tel_movil, c.tel_otro, c.uid, c.prioridad, DATE_FORMAT(c.fecha_importado,'%Y-%m-%d %h:%m:%s'), l.intentos,c.fecha_autorizado,c.fecha_firmado,l.campana_id"
    ...
    if ($db->sql_numrows($result) > 0) {
        $contador=1;
        while (list($c, $origen_id, $nombre, $apellido_paterno, $apellido_materno, $t1, $t2, $t3, $t4, $c_uid, $prioridad, $fecha_importado, $intento, $fecha_autorizado, $fecha_firmado, $id_campana) = htmlize($db->sql_fetchrow($result))) {
            if ($t4)
                $t = $t4;
            if ($t3)
                $t = $t3;

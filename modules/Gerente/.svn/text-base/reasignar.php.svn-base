        ...
        if ($db->sql_numrows ( $result ) > 0)
        {
            while ( list ( $contacto_id ) = $db->sql_fetchrow ( $result ) ) //revisar si lo mandaron en el post ( => on)
            {
                ...
                if (array_key_exists ( "$tmp", $_POST ))
                {
                    ...
                    {
                        list ( $llamada_id ) = $db->sql_fetchrow ($result2);
                    ...
                    else
                    {
                        ...
                        $sql = "INSERT INTO crm_campanas_llamadas (campana_id,contacto_id,status_id,fecha_cita)VALUES('$campana_id','$contacto_id','0','0000-00-00 00:00:00')";
                        $db->sql_query ( $sql ) or die ( "Error al leer" . print_r ( $db->sql_error () ) );
                    }
                }
            }

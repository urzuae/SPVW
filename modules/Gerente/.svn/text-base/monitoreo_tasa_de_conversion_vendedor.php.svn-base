...
global $db, $user, $uid, $fecha_ini, $fecha_fin, $submit, $fuente_id,$vendedor_id;
if ($fecha_ini)
{
  ...
  $and_fecha_imp .= " AND c.fecha_importado >'$fecha_ini 00:00:00'";
}
if ($fecha_fin)
{
  ...
  $and_fecha_imp .= " AND c.fecha_importado <'$fecha_fin 23:59:59'";
}
if($fuente_id)
{
...
if($vendedor_id){
    $and_uid.=" AND c.uid= ".$vendedor_id." ";
    $and_uid_v.=" AND v.uid= ".$vendedor_id." ";
}
...
else
{
    ...
    $sql = "SELECT fuente_id, nombre FROM crm_fuentes where fuente_id<0 ".$filtro_gid.";";
    $result = $db->sql_query($sql) or die("Error");
    ...
    $select_vendedores=Regresa_Vendedores($db,$gid,$vendedor_id);
    if ($submit)
    {
		    ...
		    WHERE v.eliminar=0 AND v.contacto_id=c.contacto_id AND c.gid='$gid'
                    $and_fecha $and_origen $and_uid_v ORDER BY v.uid,c.contacto_id";
            ...
            if($num_ventas>0)
            {
                ...
                $sql_datos="SELECT DISTINCT v.contacto_id, v.modelo_id, v.version_id, v.transmision_id,v.timestamp,v.timestamp_unidades,v.precio,v.chasis,v.uid,c.origen_id,concat(c.nombre,' ',c.apellido_paterno,' ',c.apellido_materno) as nombre,c.tel_casa,c.email,c.tel_oficina
                            FROM crm_prospectos_ventas as v,crm_contactos_finalizados AS c
                            WHERE v.eliminar=0 AND  v.contacto_id=c.contacto_id AND c.gid='$gid'
		            $and_fecha $and_origen $and_uid_v ORDER BY v.uid,nombre,apellido_paterno,apellido_materno";
            ...
            $sql = "SELECT DISTINCT v.contacto_id, v.modelo_id, v.version_id, v.transmision_id,v.timestamp,v.timestamp_unidades,v.chasis,v.precio
                    FROM crm_prospectos_ventas as v, crm_contactos AS c
		    WHERE v.eliminar=0 AND v.contacto_id=c.contacto_id AND c.gid='$gid'
                    $and_fecha $and_origen $and_uid_v ORDER BY v.uid,c.contacto_id";
        ...
        $num_capturados=0;
        $num_capturados= $num_capturados + Total_Contactos($db,'crm_contactos',$gid,$and_fecha_imp,$and_origen);
        $num_capturados= $num_capturados + Total_Contactos($db,'crm_contactos_finalizados',$gid,$and_fecha_imp,$and_origen);        
        $tasa = "0%";
        if ($num_capturados)
        ...
        if(count($array_datos) > 0)
        {
            ...
            foreach($array_datos as $contacto => $array)
            {
                ...
                $url="index.php?_module=Gerente&_op=actualiza_venta&contacto_id=$tmp_con&modelo_id=$tmp_mod&version_id=$tmp_ver&transmision_id=$tmp_tra&timestamp=$tmp_tim&chasis=$tmp_cha&fecha_ini=$fecha_ini_script&fecha_fin=$fecha_fin_script&fuente_id=$fuente_id&uid=$uid&vendedor_id=$vendedor_id&tipo_user=2";
...
function Total_Contactos($db,$tabla,$gid,$and_fecha_imp,$and_origen)
{
    $no_capturados=0;
    $sql = "SELECT COUNT(DISTINCT(c.contacto_id)) FROM ".$tabla." as c
            WHERE c.gid='".$gid."' ".$and_fecha_imp." ".$and_origen." ORDER BY c.contacto_id;";
    $r = $db->sql_query($sql) or die($sql);
    list($no_capturados) = $db->sql_fetchrow($r);
    return $no_capturados;
}
...
function Regresa_Vendedores($db,$gid,$vendedor_id)
{
    $sql = "SELECT uid, name FROM users where super=8 AND gid='".$gid."';";
    $result = $db->sql_query($sql) or die("Error");
    $select_vendedores = "<select name=\"vendedor_id\">
                     <option value=\"0\">Todos</option>";
    while(list($id, $name) = $db->sql_fetchrow($result))
    {
        ...
        if($id == $vendedor_id)
            $selected = " SELECTED";
        $select_vendedores.= "<option value='".$id."' ".$selected.">".$name."</option>";
    }
    $select_vendedores.= "</select>";
    return $select_vendedores;

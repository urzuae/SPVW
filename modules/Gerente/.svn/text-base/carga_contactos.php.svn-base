...
if ($submit)
{
    ...
    while($data = fgetcsv($fh, 1000, ","))
    {
        ...
        list($nombre, $ap_pat, $ap_mat,$titulo, $sexo,$compania, $sector, $cargo,$tel_casa, $tel_ofi, $tel_mov, $tel_otro,
            $email,$domicilio, $colonia, $delegacion, $cp, $estado, $ciudad, $pais,$rfc, $moral, $fecha_nac, $ocupacion,
            $edo_civil,$nota,$no_contactar, $primer_cont, $origen,$modelo, $version, $ano, $tipo_pint,
            $color_ext, $color_int, $vendedor,$codigo,$medio_entero) = $data2;
        if ((!$nombre))
        {
        ...
        // CHECAMOS EL MEDIO COMO SE ENTERO

        $medio_entero=strtoupper($medio_entero);
        $array=array(1 => 'RADIO',2 =>  'TV',3 =>  'PERIODICO',4 =>  'VOLANTES',
                 5 =>  'REVISTAS',6 =>  'MANTAS',7 =>  'INVITACION DE VENDEDOR');
        $medio_entero_id=array_search($medio_entero, $array);
        if($medio_entero_id== FALSE)
            $medio_entero_id=0;
        $date=date("Y-m-d H:i:s");
        $array = array('nombre' => $nombre,'apellido_paterno' => $ap_pat,'apellido_materno' => $ap_mat,'sexo' => $sexo,'compania' => $compania,'cargo' => $cargo,
'tel_casa' =>$tel_casa,'tel_oficina' =>$tel_ofi,'tel_movil' => $tel_mov,'tel_otro'  => $tel_otro,'email' => $email,'domicilio' => $domicilio,
'colonia' => $colonia,'cp' => $cp,'poblacion' => $delegacion,'entidad_id' => $entidad_id,'rfc' => $rfc,'persona_moral' => $moral,'fecha_de_nacimiento' => $fecha_nac,
'ocupacion' => $ocupacion,'edo_civil' => $edo_civil,'nota' => $nota,'no_contactar' => $no_contactar,'gid' => $gid,'uid_vendedor' => $vendedor_uid,'titulo' => $titulo,
'sector' => $sector,'pais' => $pais,'ciudad' => $ciudad,'primer_contacto'  => $primer_cont,'origen_id' => $origen,'codigo_campana' => $codigo,
'fecha_importado' => $date,'medio_entero' => $medio_entero_id, 'modelo' => $modelo,'version' => $version,
'ano' => $ano ,'tipo_pintura' =>  $tipo_pint,'color_ext' => $color_ext,'color_int' => $color_int, 'modelo_id' => $modelo_id);

        $array_contactos[]=$array;
    }
    if((count($array_contactos)> 0 )  && ($no_errores == 0))
    {
        # si no hay errores y tenemos contactos guardados en el array los insertamos en la bd
        $sql="INSERT INTO crm_contactos (
                    ...
                    ciudad, primer_contacto, origen_id,codigo_campana,fecha_importado,medio_entero)
            VALUES ('".$data['nombre']."','".$data['apellido_paterno']."','".$data['apellido_materno']."',
                    '".$data['sexo']."','".$data['compania']."','".$data['cargo']."','".$data['tel_casa']."',
                    '".$data['tel_oficina']."','".$data['tel_movil']."','".$data['tel_otro']."','".$data['email']."',
                    '".$data['domicilio']."','".$data['colonia']."','".$data['cp']."','".$data['poblacion']."',
                    '".$data['entidad_id']."','".$data['rfc']."','".$data['persona_moral']."',
                    '".$data['fecha_de_nacimiento']."','".$data['ocupacion']."','".$data['edo_civil']."',
                    '".$data['nota']."','".$data['no_contactar']."','".$data['gid']."','".$data['uid_vendedor']."',
                    '".$data['titulo']."','".$data['sector']."','".$data['pais']."','".$data['ciudad']."',
                    '".$data['primer_contacto']."','".$data['origen_id']."','".$data['codigo_campana']."',
                    '".$data['fecha_importado']."','".$data['medio_entero']."');";
            $res=$db->sql_query($sql) or die("Error al insertar contacto:  ".$sql);
            if($res)
            {
                $contacto_id =$db->sql_nextid();
                $sql_log="INSERT INTO crm_contactos_asignacion_log (contacto_id,uid,to_uid)
                    VALUES('".$contacto_id."','".$uid."','".$data['uid_vendedor']."');";
                ...
                ) VALUES ('".$contacto_id."','".$data['modelo']."','".$data['version']."','".$data['ano']."','".$data['tipo_pintura']."','".$data['color_ext']."','".$data['color_int']."','".$data['modelo_id']."');";
                $res_uni = $db->sql_query($sql_uni) or die("Error 3 <br>$sql_uni<br>");

                $sql_cam = "SELECT c.campana_id FROM crm_campanas_groups AS g, crm_campanas AS c WHERE c.campana_id=g.campana_id AND g.gid='$gid' ORDER BY c.campana_id  LIMIT 1"; 
                $result = $db->sql_query($sql_cam) or die("Error 4 <br>$sql_cam<br>");
                list($campana_id) = $db->sql_fetchrow($result);

                $sql_ins = "INSERT INTO crm_campanas_llamadas (campana_id,contacto_id,user_id,status_id,fecha_cita)VALUES('".$campana_id."','".$contacto_id."','".$data['uid_vendedor']."','0','0000-00-00 00:00:00')";
    ...
    fclose($fh);
    $file_tmp = fopen("./tmp_".date("Y-m-d").".csv" , "w" );
    if(!$file_tmp){
    ...
    fputcsv($file_tmp, split(',', "Nombre,Apellido Paterno,Apellido Materno,
                          ...
                          Modelo,Version,Año,Tipo de Pintura,Color Exterior,Color Interior,Vendedor,Codigo Campana,Medio como se entero"));
    ...
    $mensaje_contactos = "$procesados registros procesados. $insertados agregados. $no_errores errores.  ".count($alertas_motivo)." alertas.";
        ...
        if($alertas_motivo)
        {
            ...
            $tabla_alertas .= "
                <table style=\"width:100%;\">
                    ...
                    </tr></thead>$contenido_alertas
                </table>";
        }
    }


    $tabla_contenido_contactos = "
    ...
    <tbody>
        ...
        <tr class=\"\" >
            ...
            <td style=\"width:100%;\">$tabla_errores</td>
        </tr>
        <tr class=\"\" >
        ...
        <tr class=\"\" >
            <td style=\"width:15px;\"></td>
            <td style=\"width:100%;\">$tabla_alertas</td>
        </tr>
    </tbody>
    </table>";

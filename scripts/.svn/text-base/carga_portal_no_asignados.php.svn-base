...
$telefonos = array();
$alertas = $motivo_de_rechazo_1 = $motivo_de_rechazo_2 = $motivo_de_rechazo_3 = $motivo_de_rechazo_4 = 0;
...
while(1)
{
    ...
    list($fecha,$hora) = explode(" ",$fecha_de_registro);
    ...
    if ($modelo == "" || !in_array($modelo, $unidades)) //checar que el vehículo está en la lista
    {
      $rechazados[] = $linea;//linea en la que lo botamos
    ...
    $sql = "SELECT contacto_id FROM crm_contactos WHERE nombre='$nombre' AND apellido_paterno='$apellido_paterno' AND apellido_materno='$apellido_materno'";//solo checo tel_casa por que es el único que agrego en este script
    $r = $db->sql_query($sql) or die($sql);
    $num_rows_c = $db->sql_numrows($r);
    $sql = "SELECT contacto_id FROM crm_contactos WHERE nombre='$nombre' AND apellido_paterno='$apellido_paterno' AND apellido_materno='$apellido_materno'";//solo checo tel_casa por que es el único que agrego en este script
    $r_na = $db->sql_query($sql) or die($sql);
...
$msg = "Registro de asignación de prospectos no asignados desde el Portal Web de VW\nHora y fecha: ".date("H:i d-m-Y");

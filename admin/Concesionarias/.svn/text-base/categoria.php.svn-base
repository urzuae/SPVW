...
{
    $a=" groups ";
    $b=" crm_niveles_concesionarias ";
    $sql="SELECT {$a}.gid, {$a}.name, {$b}.nombre, {$b}.nivel_id FROM ( {$a} LEFT JOIN {$b} ON {$a}.gid = {$b}.gid) WHERE {$a}.gid=".$gid.";";
...
function actualiza_grupo($db,$gid,$id_nivel,$name)
{
    ...
    {
        $mensaje="<br><br><font color='3e4f88'>Se ha actualizado la concesionaria:  $name al nivel $nivel</font><br>";
    }
    return $mensaje;
}
...
function regresa_forma($id_nivel,$gid,$name)
{
    ...
    if($id_nivel==3)
        ...
        $buffer='<input type="hidden" name="gid" value="'.$gid.'">
            ...
            <tr height="30px">
            <td width="35%">Nombre de la Concesionaria: </td><td>'.$name.'</td>
            </tr>
            <tr height="30px">
            ...
            <tr height="30px">
                ...
                <input type="button" value="Permisos" onclick=location="index.php?_module=Concesionarias&_op=edit&gid='.$gid.'"></td>
           </tr>
         </table>
<br>';
    return $buffer;

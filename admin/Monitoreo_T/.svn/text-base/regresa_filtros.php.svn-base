...
$url="&grupo_empresarial_id=".$grupo_empresarial_id.
	...
	if(strlen($filtro_res) > 1)
		$leyenda_filtros.="<tr><td>Concesionaria</td><td>".$filtro_res."</td></tr>";
	...
	if(($fecha_ini != '') && ($fecha_fin != ''))
	{
		...
		$tmp_filtros[]="substr(fecha_importado,1,10) BETWEEN '".$fecha_ini."' AND '".$fecha_fin."'";
	}

    if(($fecha_ini != '') && ($fecha_fin == ''))
	{
		$leyenda_filtros.="<tr><td>Fecha </td><td>".$_REQUEST['fecha_ini']."</td></tr>";
		 $tmp_filtros[]="substr(fecha_importado,1,10)='".$fecha_ini."'";
	}
		
	if(($fecha_ini == '') && ($fecha_fin != ''))
	{
		$leyenda_filtros.="<tr><td>Fecha </td><td>".$_REQUEST['fecha_fin']."</td></tr>";	 
		$tmp_filtros[]="substr(fecha_importado,1,10)='".$fecha_fin."'";
	}	

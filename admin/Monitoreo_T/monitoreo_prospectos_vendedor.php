...
if (!defined('_IN_ADMIN_MAIN_INDEX')) {
	...
	{	
		$tabla_campanas .= "<table class=\"width100\">"
					  ...
					  ."<td style=\"width:27%;\"><a href=\"#\" onclick=\"location.href='index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gid&orderby=nombre&rsort=$nrsort&open='+document.getElementById('open').value;\" style=\"color:white;\">Nombre</a></td>"
		...
		while (list($campana_id, $name) = htmlize($db->sql_fetchrow($res_campanas)))
		{
	  		...
	  		if ($num_ciclo>0)
	  		{
			    ...
			    while (list($llamada_id, $origen_id, $contacto_id, $prospecto, $status_id, $c_uid) = $db->sql_fetchrow($res_ciclo))
	    		{     
		  			...
		  			$res_reasignacion=$db->sql_query("SELECT to_uid,to_gid,fecha_log_ultima,vendedor,fecha_importado FROM reporte_contactos_asignados WHERE contacto_id=".$contacto_id." limit 1;");

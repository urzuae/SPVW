...
$vendedores=vendedores($db,$gid,$uid);
$origenes=Origenes($db);
$unidades=Unidades($db);
$campanas=Campanas($db,$gid);
$sql="SELECT b.contacto_id,b.origen_id,concat(b.nombre,' ',b.apellido_paterno,' ',b.apellido_materno) as prospecto,b.fecha_importado,m.modelo_id FROM crm_contactos b, crm_prospectos_unidades m  WHERE b.gid=".$gid." AND b.uid=".$uid." AND b.contacto_id=m.contacto_id  ".$filtro_contacto."  ".$filtro_vehiculo." GROUP by b.contacto_id ORDER BY b.contacto_id;";
...
if( $num > 0)
{
    ...
    {
        Crea_tabla_temporal($db,$contacto_id);
...
function Regresa_Prospectos_en_Atencion($db,$contacto_id)
{
    ...
    $sql_asi="SELECT TIMESTAMPDIFF(HOUR,a.timestamp,now()) as hrs_cita_atencion
         FROM unicos_log as a WHERE  a.contacto_id IN (SELECT b.contacto_id FROM temporal_p b WHERE b.status=0 GROUP BY a.gid);";
...
function Crea_tabla_temporal($db,$contacto_id)
{
    $db->sql_query("DROP TABLE IF EXISTS temporal_p;");
    $db->sql_query("CREATE TABLE temporal_p as SELECT DISTINCT c.contacto_id,b.gid,b.uid,c.id,c.user_id,c.fecha_cita,0 as status,0000000000 as evento_id,0000000000 as cierre_id FROM crm_contactos b,crm_campanas_llamadas c WHERE b.contacto_id=".$contacto_id." AND b.contacto_id=c.contacto_id ;");

}
function Elimina_tabla_temporal($db)
{
    $db->sql_query("DROP TABLE temporal_p;");
}
function Regresa_Prospectos_en_Seguimiento($db,$contacto_id)
{
    $array_compromiso=array();
    $upd_seguimiento    = "update temporal_p as c set c.status=1 WHERE c.id in (select b.llamada_id from crm_campanas_llamadas_eventos b where b.uid=c.uid and c.contacto_id=".$contacto_id.");";
    $res_upd_seguimiento=$db->sql_query($upd_seguimiento);
    $no_prospectos_compromiso=$db->sql_affectedrows($res_upd_seguimiento);
    $upd_evento_id="update temporal_p as c set c.evento_id=(select distinct b.evento_id FROM crm_campanas_llamadas_eventos b where b.uid=c.uid and c.id=b.llamada_id order by evento_id desc limit 1) WHERE c.status=1;";
    $db->sql_query($upd_evento_id);
    $upd_cierre ="update temporal_p as c set c.cierre_id=(select distinct b.cierre_id FROM crm_campanas_llamadas_eventos_cierres b where b.uid=c.uid and c.evento_id=b.evento_id order by b.cierre_id desc limit 1) WHERE c.status=1;";
    $db->sql_query($upd_cierre);
    $udp_no_asignados   = "update temporal_p as  c set c.status=2 WHERE c.uid=0;";
    $db->sql_query($udp_no_asignados);
    $sql_compromiso="select b.fecha_cita as cita_atrazada,TIMESTAMPDIFF(HOUR,b.fecha_cita,now()) as hrs_cita_atrazada from temporal_p b where b.contacto_id=".$contacto_id." and b.status=1 and b.cierre_id=0 and b.fecha_cita<=now();";
...
function vendedores($db,$gid,$uid)
{
    $sql_tmp="SELECT uid,name from users WHERE gid='".$gid."' AND uid=".$uid." ORDER BY uid;";

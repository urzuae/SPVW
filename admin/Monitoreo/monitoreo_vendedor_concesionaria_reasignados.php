...
global $db, $gid, $from, $campana_id, $uid, $orderby,$tmp_filtros,$order,$url,$_dbhost,$_dbuname,$_dbpass,$_dbname;
...
$result = mysqli_query($linkTodb,"CALL reasignaciones_vendedores('$gid','$tmp_origen','$fecha_i','$fecha_c')");
if (! $result)
{
...
if(mysqli_num_rows($result) > 0)
{
    ...
    while (list($uid,$no_contactos,$no_reasignaciones,$maximos) = mysqli_fetch_array($result,MYSQLI_NUM))
    {
        $gid=str_pad($gid, 4, "0", STR_PAD_LEFT);
        $array_reasignaciones[$uid]['no_cont']=$no_contactos;
        $array_reasignaciones[$uid]['no_reas']=$no_reasignaciones;
        $array_reasignaciones[$uid]['maximos']=$maximos;

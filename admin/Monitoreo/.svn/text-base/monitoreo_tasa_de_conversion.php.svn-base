...
if ($fecha_ini)
{
  ...
  $and_fecha_imp .= " AND c.fecha_importado >='$fecha_ini 00:01:01'";
...
$sql = "SELECT fuente_id, nombre FROM crm_fuentes where fuente_id<0";
...
if ($submit)
{
    ...
    $sql = "SELECT COUNT(DISTINCT(c.contacto_id)) FROM crm_contactos as c WHERE c.contacto_id > 0 $and_fecha_imp $and_origen $and_gid";
    $r = $db->sql_query($sql) or die($sql);
    list($num_capturados) = $db->sql_fetchrow($r);

    $sql_fin = "SELECT COUNT(DISTINCT(c.contacto_id)) FROM crm_contactos_finalizados as c WHERE c.contacto_id > 0 $and_fecha_imp $and_origen $and_gid";
    ...
    $sql = "SELECT DISTINCT c.contacto_id, v.modelo_id, v.version_id, v.transmision_id ,v.timestamp,v.timestamp_unidades,v.chasis,v.precio
            ...
            WHERE v.eliminar=0 AND v.contacto_id=c.contacto_id $and_fecha $and_origen $and_gid"; //OR gid='0'
    $r = $db->sql_query($sql) or die($sql);
    ...
    if($num_ventas>0)
    {
        ...
        $sql_datos="SELECT DISTINCT c.contacto_id, v.modelo_id, v.version_id, v.transmision_id,v.timestamp,v.timestamp_unidades,v.chasis,v.precio,v.uid,c.origen_id,concat(c.nombre,' ',c.apellido_paterno,' ',c.apellido_materno) as nombre,c.tel_casa,c.email,c.tel_oficina
		        ...
		        WHERE v.eliminar=0 AND v.contacto_id=c.contacto_id $and_fecha $and_origen $and_gid  ORDER BY nombre,apellido_paterno,apellido_materno";
    ...
    $sql = "SELECT DISTINCT c.contacto_id, v.modelo_id, v.version_id, v.transmision_id,v.timestamp,v.timestamp_unidades,v.chasis,v.precio
            ...
            WHERE v.eliminar=0 AND v.contacto_id=c.contacto_id $and_fecha $and_origen $and_gid";
    $r = $db->sql_query($sql) or die($sql);

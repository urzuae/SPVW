...
if (!$contacto_id)//chekamos si tenemos un usuario o tenemos ke eskogerlo de forma inteligente
{
    ...
    if (!$nopendientes) //solo entrar aki si no esta seteado, osea, entra a buscar siempre que se pueda
    {
        ...
        {
            $lista = "<table>\n<thead><td>ssssNombre</td><td>Paterno</td><td>Materno</td><td>Cita</td><td></td></thead>";
...
// Desccomenentar la siguiente linea en caso que se quiera tener activo el lock
//if (!(($lock == 0) || ($lock && ($dia_ts != $hoy)) || ($lock && ($uid_lock==$uid)))) die("<script>setTimeout('location.href=\"index.php?_module=$_module&_op=llamada_ro&campana_id=$campana_id&nopendientes=1\"',3000);</script><center>Error: este registro está siendo usado.<br><a href=\"index.php?_module=$_module&_op=llamada&campana_id=$campana_id&nopendientes=1\">Continuar</a></center>");
...
$sql = "SELECT
                    ...
                    tel_movil, tel_otro,
                    nota, email, origen_id, tel_casa_2, tel_oficina_2, tel_movil_2,
                    horario_preferido_casa,horario_preferido_oficina,horario_preferido_movil,
                    horario_preferido_casa_2,horario_preferido_oficina_2,horario_preferido_movil_2,codigo_campana
...
list($nombre, $apellido_paterno, $apellido_materno,$tel_casa, $tel_oficina,$tel_movil, $tel_otro,$nota, $email,
    ...
    $horario_preferido_movil,$horario_preferido_casa_2,$horario_preferido_oficina_2,$horario_preferido_movil_2,$codigo_campana) = htmlize($db->sql_fetchrow($result));
/** calculo para los checkbox **/
$d_casa=revisa_disponibilidad($horario_preferido_casa);
$d_oficina=revisa_disponibilidad($horario_preferido_oficina);
$d_movil=revisa_disponibilidad($horario_preferido_movil);
$d_casa_2=revisa_disponibilidad($horario_preferido_casa_2);
$d_oficina_2=revisa_disponibilidad($horario_preferido_oficina_2);
$d_movil_2=revisa_disponibilidad($horario_preferido_movil_2);

//datos del vehículo
$buffer_modelo='';
$sql = "SELECT modelo,version,paquete ,tipo_pintura,color_exterior,color_interior,modelo_id,version_id,transmision_id,timestamp
...
if($db->sql_numrows($result) > 0)
{
    ...
    while ($fila = $db->sql_fetchrow($result))
    {
        $tmp_m[]=$fila['modelo']."";
        $tmp_p[]=$fila['paquete']."";
        $tmp_t[]=$fila['tipo_pintura']."";
        $tmp_ce[]=$fila['color_exterior']."";
        $tmp_ci[]=$fila['color_interior']."";
        ...
        if($db->sql_numrows($res_chasis) > 0)
            $nombre_modelo=$db->sql_fetchfield(0,0,$res_chasis);

        $tmp_ch[$tmp_contador]=$nombre_modelo;
        $tmp_contador++;
    }
    ...
    if(count($tmp_m) > 0)
    {
        ...
        $array_titulos=array(0 => 'Modelo',1 => 'Versi&oacute;n',2 => 'Tranmisi&oacute;n',3 => 'Tipo Pintura',
        4 => 'Color Exterior',5 => 'Color Interior',6 => 'Chasis');
...
/*
$sql = "SELECT modelo,version_id,paquete ,tipo_pintura,color_exterior,color_interior
                FROM crm_prospectos_unidades WHERE contacto_id='$contacto_id' LIMIT 1";
$result = $db->sql_query($sql) or die("Error al consultar datos del contacto".$sql);
list($modelo,$version_id,$paquete,$tipo_pintura,$color_exterior,$color_interior
...
$sql_chasis="SELECT chasis FROM crm_prospectos_ventas WHERE contacto_id='$contacto_id' LIMIT 1";
$res_chasis=$db->sql_query($sql_chasis);
$num_chasis=$db->sql_fetchfield('chasis',0,$res_chasis);
if(!empty($num_chasis))
    $num_chasis=$num_chasis."  Venta Finalizada";*/
...
/*
$catalogo = "\n<ul id=nav>\n";
$result = $db->sql_query("SELECT art_id, nombre FROM crm_catalogo WHERE parent_id='0' ORDER BY nombre") or die("Error en catalogo");
while (list($art_id, $nombre_art) = $db->sql_fetchrow($result))
{

    if (file_exists("admin/Catalogo de Productos/files/$art_id.jpg"))
    {
                $link_img = " href=\"javascript:pop_catalogo('admin/Catalogo de Productos/files/".$art_id.".jpg','$nombre_art','jpg');\"";
    }
    else if (file_exists("admin/Catalogo de Productos/files/$art_id.pdf"))
    {
                $link_img = " href=\"javascript:pop_catalogo('admin/Catalogo de Productos/files/".$art_id.".pdf','$nombre_art','pdf');\"";
    }
    else $link_img = "";
    $catalogo .= " <li>\n  <a$link_img>$nombre_art</a>\n";
    $result2 = $db->sql_query("SELECT art_id, nombre FROM crm_catalogo WHERE parent_id='$art_id' ORDER BY nombre")
        or die("Error en catalogo");
    if ($db->sql_numrows($result2) > 0)
    {
        $catalogo .= "  <ul>\n";
        while (list($art_id, $nombre_art) = $db->sql_fetchrow($result2))
        {
            if (file_exists("admin/Catalogo de Productos/../img/$art_id"))
            {
                $link_img = " href=\"javascript:pop_catalogo('admin/Catalogo de Productos/../img/".$art_id.".jpg','$nombre_art');\"";
            }
            else $link_img = "";
            $catalogo .= "   <li><a$link_img>$nombre_art</a></li>\n";
        }
        $catalogo .= "  </ul>\n";
    }
    $catalogo .= " </li>\n";
...
$cat_productos = array("Pointer" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/POinter2.html",
                  "Lupo" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/fox.html",
                  "Derby" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/derby.html",
                  "Polo" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/polo.html",
                  "SportsVan" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/multivan.html",
                  "CrossFox" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/Fox.html",
                  "Jetta" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/jetta.html",
                  "Golf" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/golfmx.html",
                  "Beetle" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/beetle_mx.html",
                  "Bora" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/touran.html",
                  "GLI" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/gli_fah.html",
                  "GTI" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/gti_mx.html",
                  "Passat" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/passat_mx.html",
                  "Sharan" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/sharan.html",
                  "Touareg" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/models/touareg.html",
				  "Promociones" => "http://www.volkswagen.com/vwcms_publish/vwcms/master_public/virtualmaster/es_mx/");
$catalogo = "\n<ul id=nav>\n";
...
{
//      $objecion = nl2br($objecion);
//     $objecion = strtr($objecion, "\n", " ");
//     $objecion = strtr($objecion, "\r", " ");
...
//   $cerrar_evento .= "<input name=\"evento_cierre_comentario\"  style=\"width:180px;\" onblur=\"caps1(this);\"><br><br>";
  $cerrar_evento .= "";
...
function revisa_disponibilidad($campo)
{
    switch(strtolower($campo))
    {
        case "mtn":
            $m=" checked "; $t=" checked "; $n=" checked ";
            break;
        case "mt":
            $m=" checked "; $t=" checked "; $n=" ";
            break;
        case "mn":
            $m=" checked "; $casa_t=" "; $casa_n=" checked ";
            break;
        case "tn":
            $m=" "; $t=" checked "; $n=" checked ";
            break;
        case "m":
            $m=" checked "; $t=""; $n="";
            break;
    ...
    $buffer='
        <table width="100%">
        <tr><td><input type="checkbox" name="1" readonly '.$m.'></td>
        <td><input type="checkbox" name="1" readonly '.$t.'></td>
        <td><input type="checkbox" name="1" readonly '.$n.'></td></tr>
        </table>';
    return $buffer;
}
?>
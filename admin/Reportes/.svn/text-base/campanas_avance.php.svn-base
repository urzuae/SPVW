...
global $db, $encuesta_id, $descripcion, $nombre, $submit, $gid,$fecha_ini,$fecha_fin;
...
if($submit){
  ...
  fputcsv($fp,$titulo);
  $row = array("Campaña", "Concesionarias");
  ...
  fputcsv($fp, $row);
  if($gid){
    $sql = "SELECT campana_id, nombre FROM crm_campanas WHERE LEFT(nombre,4) = '$gid' ORDER BY campana_id";
  }
  else{
    $sql = "SELECT campana_id, nombre FROM crm_campanas WHERE 1 ORDER BY campana_id";
  }

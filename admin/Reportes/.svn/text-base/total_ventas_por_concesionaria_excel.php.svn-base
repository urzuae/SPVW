...
if ($gid) {
    ...
    $worksheet->write ( 0, 0, sprintf ( "REPORTE CONCESIONARIA %s %s (%s)", $gid, $grupo, $titulo ), $header_format );
    $worksheet->write ( 1, 0, "Nombre", $titles_format );
    $worksheet->write ( 1, 1, "Total", $titles_format );
    $reporte = "Concesionaria";
} else {
    $worksheet->setColumn ( 0, 0, 5 ); // Zona
    $worksheet->setColumn ( 1, 1, 40 ); // Nombre
    $worksheet->write ( 0, 0, sprintf ( "REPORTE VENTAS POR CONCESIONARIA (%s)", $titulo ), $header_format );
    $worksheet->write ( 1, 0, "#", $titles_format );
    $worksheet->write ( 1, 1, "Concesionaria", $titles_format );

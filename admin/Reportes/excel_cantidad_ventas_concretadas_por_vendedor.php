<?php
    ...
    $total=0;
    $sql = "SELECT u.gid, u.name, count(v.uid) FROM `crm_prospectos_ventas` AS v, users AS u where v.uid=u.uid $and_fecha group by (v.uid)";
    ...
    if ($db->sql_numrows($result) > 0)
    {
        $table="
            ...
            <tbody>
                <tr><td colspan='3' align='center'>Cantidad de Ventas Concretadas por Vendedor/<td></tr>
            </tbody>
            <thead>
            <tr bgcolor='#cdcdcd'>
                <th align='center'>Id Concesionaria</th>
                <th align='center'>Nombre del Vendedor</th>
                <th align='center'>Ventas Concretas</th>
            </tr>
            </thead><tbody>";
        ...
        $table.="</tbody>
            <thead><tr bgcolor='#cdcdcd'><th colspan='2' align='right'>Total de Ventas:</th><th align='rigth'>".$total."</th></tr></thead></table>";
        $objeto = new Genera_Excel($table,'Ventas_Concretadas');

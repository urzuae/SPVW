...
if ($submit)
{
    ...
    global $db;
    $array_contactos_no_asignados=array();
    ...
    if($error==0)
    {
        ...
        while(1)
        {
            ...
            elseif ($linea > 4)
            {
                ...
                list($nombre, $apellidos,$email,$lada, $telefono,$modelo,$medio_contacto,$estado,$ciudad,
                     ...
                     $horario_preferido_movil,$lada_movil_2,$telefono_movil_2,$horario_preferido_movil_2,$medio_entero)= $data2;
                ...
                if (!in_array(strtoupper(trim($modelo)), $unidades)) //checar que el vehí­culo está en la lista
                {
                    $rechazados[] = $linea;//linea en la que lo botamos
                ...
                $modelo_id=$unidades[strtoupper(trim($modelo))] + 0;
                ...
                $array=array(1 => 'RADIO',2 =>  'TV',3 =>  'PERIODICO',4 =>  'VOLANTES',5 =>  'REVISTAS',6 =>  'MANTAS',7 =>  'INVITACION DE VENDEDOR');
                ...
                $array = array('nombre' => $nombre,'apellido_paterno' => $apellido_paterno,'apellido_materno' => $apellido_materno,
                    'tel_casa' =>$tel_casa,'email' => $email,'persona_moral' => $persona_moral,'domicilio' => $estado,
                    'poblacion' => $ciudad, 'fecha_importado' => $fecha_importado,'gid'=> $gid,
                    'entidad_id' => $entidad_federativa_id, 'origen_id' => $origen_id,'prioridad' => $prioridad,
                    'medio_contacto' => $medio_contacto,'medio_entero' => $medio_entero_id,'tel_casa_2' =>$tel_casa_2,
		    'tel_oficina' =>$tel_oficina,'tel_oficina_2' =>$tel_oficina_2,'tel_movil' =>$tel_movil,'tel_movil_2' =>$tel_movil_2,
		    'horario_preferido_casa' =>$horario_preferido_casa,'horario_preferido_oficina' =>$horario_preferido_oficina,
		    'horario_preferido_movil' =>$horario_preferido_movil,'horario_preferido_casa_2' =>$horario_preferido_casa_2,
	            'horario_preferido_oficina_2' =>$horario_preferido_oficina_2,'horario_preferido_movil_2' =>$horario_preferido_movil_2,
	            'modelo' => $modelo,'modelo_id' => $modelo_id);
        ...
        if((count($array_contactos_no_asignados)>0) && (count($rechazados) == 0))
        {
            for($i=0; $i < count($array_contactos_no_asignados); $i++)
            {
               $data=$array_contactos_no_asignados[$i];
               $sql = "INSERT INTO crm_contactos_no_asignados (apellido_paterno,apellido_materno,nombre,tel_casa,email,
                        persona_moral,domicilio,poblacion,fecha_importado,fecha_alta,gid,entidad_id,origen_id,
                        prioridad,medio_contacto,medio_entero,tel_casa_2,tel_oficina,tel_oficina_2,tel_movil,tel_movil_2,
                        ...
                        VALUES ('".$data['apellido_paterno']."','".$data['apellido_materno']."','".$data['nombre']."',
                        '".$data['tel_casa']."','".$data['email']."','".$data['persona_moral']."','".$data['domicilio']."',
                        '".$data['poblacion']."','".$data['fecha_importado']."','".$data['fecha_alta']."','".$data['gid']."',
                        '".$data['entidad_id']."','".$data['origen_id']."','".$data['prioridad']."','".$data['medio_contacto']."',
                        '".$data['medio_entero']."','".$data['tel_casa_2']."','".$data['tel_oficina']."','".$data['tel_oficina_2']."',
                        '".$data['tel_movil']."','".$data['tel_movil_2']."','".$data['horario_preferido_casa']."',
                        '".$data['horario_preferido_oficina']."','".$data['horario_preferido_movil']."',
                        '".$data['horario_preferido_casa_2']."','".$data['horario_preferido_oficina_2']."',
                        '".$data['horario_preferido_movil_2']."')";
                    ...
                    if($r2)
                    {
                        ...
                        //insertar modelo
                        $sql = "insert into crm_prospectos_unidades_no_asignados (contacto_id, modelo,modelo_id)VALUES('$contacto_id', '".$data['modelo']."','".$data['modelo_id']."')";
                        $r2 = $db->sql_query($sql) or die("Error<br>$sql<br>".print_r($db->sql_error()));
                    }
            }
        }
        ...
        if ($asignados_por_gid)
        {
            $msg .= "<thead>
	         <tr>
			   <td colspan=\"2\">Cantidad asignada por concesionaria</td>
			 </tr>
			 <tr>
			   <td>Concesionaria</td>
			   <td>Cantidad</td>
        ...
        if ($rechazados)
        {
            ...
            $msg .= "<tr class=\"row1\">
			 ...
			 <tr class=\"row1\">
	           <td>Rechazados por presentar una concesionaria inválida</td>
        ...
        if ($rechazados_concesionaria)
        {
            $msg .= "<thead>
	           <tr>
			     <td colspan=\"2\">Concesionarias no encontradas en la BD, requieren atención</td>
			   </tr>
			   <tr>

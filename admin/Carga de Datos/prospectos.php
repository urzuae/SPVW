...
$insertados2 = 0;
$prioridades = array();
$sql = "SELECT prioridad,prioridad_id, valor FROM crm_prioridades_contactos";
$r = $db->sql_query($sql) or die($sql);
while (list($nm_prioridad,$prioridad_id, $prioridad_val) = $db->sql_fetchrow($r)) {
    ...
    if ($error == 0) {
        ...
        while (1)
        {
            ...
            elseif ($linea > 4) {
                ...
                // valido la fecha de importado
                $sql="SELECT year('".$fecha_importado."') as year,month('".$fecha_importado."') as month, day('".$fecha_importado."') as day";
                $res=$db->sql_query($sql);
                list($import_year, $import_month, $import_day) = $db->sql_fetchrow($res);
                ...
                $modelo_id = Revisa_modelo($db, ucwords(trim($modelo)));
                ...
                $array = array('nombre' => $nombre,'apellido_paterno' => $apellido_paterno,'apellido_materno' => $apellido_materno,
                    ...
                    'poblacion' => $ciudad, 'fecha_importado' => $fecha_importado,'fecha_alta' => $date, 'gid'=> $gid,
                    'entidad_id' => $entidad_federativa_id, 'origen_id' => $origen_id,'prioridad' => $prioridad,
                    'medio_contacto' => $medio_contacto,'medio_entero' => $medio_entero_id,'modelo' => $modelo,'modelo_id' => $modelo_id);

                $array_no_asignado=array();
                ...
                if ($rs[0] > 0)
                    $array_no_asignados[]=$array;
                else
                    $array_asignados[]=$array;
            }
        }
        fclose($fh); //ya se leyo todo del archivo
        if((count($array_asignados) > 0) && (count($rechazados) == 0))
        {
            for($i=0; $i < count($array_asignados); $i++)
            {
               $data=$array_asignados[$i];
               $sql = "INSERT INTO crm_contactos (apellido_paterno,apellido_materno,nombre,tel_casa,email,
                        ...
                        prioridad,medio_contacto,medio_entero)
                        VALUES ('".$data['apellido_paterno']."','".$data['apellido_materno']."','".$data['nombre']."',
                                ...
                                '".$data['entidad_id']."','".$data['origen_id']."','".$data['prioridad']."','".strtolower($data['medio_contacto'])."','".$data['medio_entero']."')";
                ...
                if($r2)
                {
                    ...
                    $sql = "INSERT INTO crm_contactos_asignacion_log (contacto_id, uid, from_uid, to_uid, from_gid, to_gid)VALUES('$contacto_id','0','0','0','0','".$data['gid']."')";
                    $db->sql_query($sql) or die("Error");

                    $sql = "insert into crm_prospectos_unidades (contacto_id, modelo,modelo_id)VALUES('$contacto_id', '".$data['modelo']."','".$data['modelo_id']."')";
                    $r2 = $db->sql_query($sql) or die("Error<br>$sql<br>" . print_r($db->sql_error()));
                    $insertados1++;
                    $insertados++;
                }
        ...
        if((count($array_no_asignados) > 0) && (count($rechazados) == 0))
        {
            for($i=0; $i < count($array_no_asignados); $i++)
            {
               $data=$array_no_asignados[$i];
               $sql = "INSERT INTO crm_contactos_no_asignados (apellido_paterno,apellido_materno,nombre,tel_casa,email,
                        ...
                        '".$data['tel_casa']."','".$data['email']."','".$data['persona_moral']."','".$data['domicilio']."',
                        '".$data['poblacion']."','".$data['fecha_importado']."','".$data['fecha_alta']."','".$data['gid']."',
                        '".$data['entidad_id']."','".$data['origen_id']."','".$data['prioridad']."','".$data['medio_contacto']."','".$data['medio_entero']."')";
                $r2 = $db->sql_query($sql) or die("Error<br>$sql<br>" . print_r($db->sql_error()));
                if($r2)
                {
                    $contacto_id = $db->sql_nextid($r2);

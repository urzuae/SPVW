...
if ($opcion_id) //ya habia algo en la tabla relacionado con esta opcion
{
    ...
    {
        $sql = "UPDATE crm_encuestas_preguntas_tipo_2 SET valor='$valor', opcion='$opcion' WHERE opcion_id='$opcion_id'";
        $db->sql_query($sql) or die("Error al insertar. ".print_r($db->sql_error()));
        header("location:index.php?_module=$_module&_op=pregunta_tipo_2&pregunta_id=$pregunta_id");
    }
    else 
    {
    //ahora las opciones que ya estan en la DB
      $sql = "SELECT opcion_id, opcion, valor, pregunta_id FROM crm_encuestas_preguntas_tipo_2 WHERE opcion_id='$opcion_id'";
...
else //nuevo
{
    ...
    {
        $sql = "INSERT INTO crm_encuestas_preguntas_tipo_2 (pregunta_id, valor, opcion)VALUES('$pregunta_id', '$valor', '$opcion')";

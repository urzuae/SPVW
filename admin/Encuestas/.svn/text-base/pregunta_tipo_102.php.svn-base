...
$sql = "SELECT valor FROM crm_encuestas_preguntas_tipo_102 WHERE pregunta_id='$pregunta_id'"; //checar si existe o si no insertar
$result = $db->sql_query($sql) or die("Error al leer.".print_r($db->sql_error()));
...
if ($db->sql_numrows($result) > 0) //ya habia algo en la tabla relacionado con esta pregunta
{
    ...
    {
        $sql = "UPDATE crm_encuestas_preguntas_tipo_102 SET valor='$valor' WHERE pregunta_id='$pregunta_id'";
...
else //nuevo
{
    ...
    {
        $sql = "INSERT INTO crm_encuestas_preguntas_tipo_102 (pregunta_id, valor)VALUES('$pregunta_id', '$valor')";

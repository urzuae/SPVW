...
switch($action){
    case "grupo_empresarial_id":{
        ...
        while($rs = $db->sql_fetchrow($res))
			echo '<option value="'.$rs["region_id"].'">'.htmlentities($rs["nombre"]).'</option>';
	break;
    }
	...
	case "zona_id":{
		...
		while($rs = $db->sql_fetchrow($res))
        {
			echo '<option value="'.$rs["entidad_id"].'">'.htmlentities($rs["nombre"]).'</option>';
        }
    break;
	}
    case "entidad_id":{
		...
		echo"<option value='0'>Selecciona</option>";
		while($rs = $db->sql_fetchrow($res))
			echo '<option value="'.$rs["plaza_id"].'">'.htmlentities($rs["nombre"]).'</option>';
	break;
    ...
    case "plaza_id":{
		...
		echo"<option value='0'>Selecciona</option>";
		while($rs = $db->sql_fetchrow($res))
			echo '<option value="'.$rs["gid"].'">'.htmlentities($rs["name"]).'</option>';
        break;
        }
	...
	case "origenPadre":{				
		...
		echo"<option value='0'>Selecciona</option>";		
		while($rs = $db->sql_fetchrow($res))
			echo '<option value="'.$rs["origen_id"].'">'.htmlentities($rs["nombre"]).'</option>';	
	break;
	}

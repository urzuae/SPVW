...
global $db,$fuente_id,$guardar,$busca,$bus_gid,$bus_nom,$seleccionados;
...
if($guardar)
{
    ...
    $actualizadas="<font color='#3e4f88'>".$actualizaciones." concesionarias <b>NO VISIBLES</b> para la fuente ".$nom_fuente."</font>";
}
...
$tabla="<br><br><table width='80%' align='center' border='0'>
        <tr>
            <td width='22%' align='left' valign='top'>Buscar Concesionaria por ID:</td>
        ...
        <tr>
            <td colspan='3' align='center' valign='top'><font color='#800000' size='2'><b>Desmarque</b> las concesionarias en las que <b>desea</b> que se visualize la fuente: </font><b>".$nom_fuente."</b><br></td>
        </tr>
        <tr height='30'>
...
{
    $buffer='No Existen Concesionarias';
    $sql="SELECT gid,name FROM groups WHERE active=1 and gid>3 ".$filtro." ORDER BY gid;";
    $res=$db->sql_query($sql);
    $num=$db->sql_numrows($res);
    if($num >0)
    {
        $contador=0;
        $buffer="<table align='center' class='tablesorter'>
        ...
        while(list($gid,$name)=$db->sql_fetchrow($res))
        {
            ...
            $tmp=' ';
            $titulo="<font color='#006600'>Activa</font>";
            $val=0;
            $jsname="g".$gid;
            if(in_array($gid,$array))
            {
                $tmp=' checked ';
                $titulo="<font color='#800000'>Bloqueado</font>";
                $val=1;
            }

            $buffer.="<tr bgcolor='".$color."'>";
            $buffer.="<td align='left'><input type='checkbox' name='".$jsname."'  id='".$jsname."' value='".$gid."' ".$tmp.">$titulo</td><td>".$gid."</td><td>".$name."</td></tr>";
            $contador++;
        }

...
global $db,$fuente_id,$submit,$xgid,$orden,$fecha_ini,$fecha_fin;

if($submit)
{
    $array_buf= Genera_BD($db,$fuente_id,$xgid,$fecha_ini,$fecha_fin,$orden);
    $buf_excel=$array_buf[0];
    $buffer=$array_buf[1];
    $file="archivos/bd_excel".date("Y-m-d").".xls";
    $f1=fopen($file,"w+");
    fwrite($f1,$buf_excel);
    fclose($f1);
...
$buf="<table width='100%' align='center' style='border:1px solid #cdcdcd'>
        <tbody>
            <tr><td>Fuente:</td><td colspan='3'>".Genera_Combo_Fuentes($db,$fuente_id)."</td></tr>
            <tr><td>Concesionaria:</td><td colspan='3'>".Genera_Combo_Concesionarias($db,$xgid)."</td></tr>
            <tr>
                <td>Fecha Inicial:</td>
                <td>
            ...
            <tr>
                <td>Ordenado por</td>
                <td><input type='radio' name='orden' id='orden' value='1' checked>Nombre</td>
                <td><input type='radio' name='orden' id='orden' value='2'>Concesionaria</td>
            </tr>
            <tr><td colspan='3' align='center'><br><input type='submit' name='submit' value='Generar Excel'></td></tr>
            <tr><td colspan='3' align='center'><br>".$buffer."</td></tr>
        </tbody></table>";

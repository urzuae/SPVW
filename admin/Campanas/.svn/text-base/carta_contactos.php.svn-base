...
if ($reset)
{
  $sql = "DELETE FROM crm_campanas_cartas_contactos WHERE campana_id='$campana_id'";
  $result = $db->sql_query($sql) or die(print_r($db->sql_error()));
...
if ($submit || $guardar) //buscando
{
            ...
            $db->sql_fetchrow($result))
    {
        if ($guardar) //GUARDAR!
        {
            $sql = "SELECT 1 FROM crm_campanas_cartas_contactos WHERE contacto_id='$contacto_id' AND campana_id='$campana_id'";
            if ($db->sql_numrows($db->sql_query($sql)) == 0) //si no está ya, agregar
            {
                $sql = "INSERT INTO crm_campanas_cartas_contactos (campana_id, contacto_id) VALUES('$campana_id','$contacto_id')";
...
$sql = "SELECT contacto_id FROM crm_campanas_cartas_contactos WHERE campana_id='$campana_id'";

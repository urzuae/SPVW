...
function contactar($name,$si)
{    
    $array=array('' => 'Seleccione uno','0' => 'No contactar', '1' => 'Si contactar',
    '2' => 'No contactar por e-mail','3' => 'No contactar por Correo', '4' => 'No contactar por teléfono',
    '5' => 'No contactar por SMS');

    $select = "<select name=\"$name\" id=\"$name\">\n";
    foreach($array as $id =>$titulo)
    {
        $tmp='';
        if($id != '')
        {
            if($id == $si) $tmp=' SELECTED ';
        }
        $select.="<option value='".$id."' ".$tmp.">".$titulo."</option>";
    }
    $select.="</selected>";
    return $select;
}
...
function select_medio_entero($medio_id,$opc)
{
    $style='';
    if($opc == 1)
    {
        $style=" style='background:#fff;width:445px;border:1px solid #a9bfd3;color:#333333;'";
    }
    $array=array(0 => 'Selecciona',1 => 'Radio',2 =>  'TV',3 =>  'Periódico',4 =>  'Volantes',
                 5 =>  'Revistas',6 =>  'Mantas',7 =>  'Invitación de vendedor');
    $select="<select name=\"medio_entero_id\" id=\"medio_entero_id\" ".$style.">";

...
function Genera_combo_Vendedores_Multiple($db,$gid)
{
    $buffer.="<select name='listado_uid[]' id='listado_uid' multiple='multiple' style='width:450px; height:90px; border:1px solid #cdcdcd;color:#000000;'>
                  <option value='0'></option></select>";
    return $buffer;
}

function Genera_combo_Concesionarias_Participantes_Multiple($db,$gid)
{
    $buffer.="<select name='listado_gid_p[]' id='listado_gid_p' multiple='multiple'  style='width:450px; height:90px; border:1px solid #cdcdcd;color:#000000;'></select>";
    return $buffer;

}
function Genera_combo_Vendedores_Participantes_Multiple($db,$gid)
{
    $buffer.="<select name='listado_uid_p[]' id='listado_uid_p' multiple='multiple' style='width:450px; height:90px; border:1px solid #cdcdcd;color:#000000;'></select>";
    return $buffer;

}

function Genera_combo_Concesionarias_Multiple($db,$gid)
{
    $buffer='';
    $sql="SELECT gid,name FROM groups WHERE gid> 3 ORDER BY gid;";
    $res=$db->sql_query($sql);
    if($db->sql_numrows($res) > 0)
    {
        $buffer.="<select name='listado_gid[]' id='listado_gid' multiple='multiple' style='width:480px; height:130px; border:1px solid #cdcdcd;color:#000000;'>";
        while(list($id,$nombre) = $db->sql_fetchrow($res))
        {
            $tmp='';
            if($id == $gid)   $tmp=' SELECTED ';
...
{
    $buffer.="<select name='tmp_uid' id='tmp_uid' style='width:260px; border:1px solid #cdcdcd;color:#000000;'></select>";
    return $buffer;
}

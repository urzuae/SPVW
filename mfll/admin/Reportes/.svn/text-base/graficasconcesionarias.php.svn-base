...
$sql_count="SELECT COUNT(a.uid) as total_f FROM mfll_contactos_registro as a WHERE 1 ".$rango_fechas." AND a.gid=".$id.";";
$sql="SELECT a.uid as ids ,count(a.uid) as total FROM mfll_contactos_registro as a WHERE 1 ".$rango_fechas." AND a.gid=".$id." GROUP BY a.uid ORDER BY a.uid;";
...
$url_reg="index.php?_module=Reportes&_op=graficas&tipo=".$tipo."&fecha_ini=".$fecha_ini."&fecha_fin=".$fecha_fin."&evento_id=".$evento_id."&evento_id=".$evento_id."&submit=sumbit";

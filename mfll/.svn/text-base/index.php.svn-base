...
function _do_module($_module) {
	...
	global $PHP_SELF, $_includesdir, $_modulesdir, $_op, $_theme, $_html, $_htmldir, $_module, $_site_title,
            $_themedir;
...
function _ask_to_login($message)
{
    ...
    print("<div class=title>$message   </div><br>"
        ...
        . "<tr><td>Usuario</td><td><input type=\"text\" name=\"_user\" onblur=\"\"></td></tr>\n"
        . "<tr><td>Password</td><td><input type=\"password\" name=\"_password\"></td></tr>\n"
        . "<tr><td colspan=2 align=center><input type=\"submit\" name=\"_do_login\" value=\"Login\"></td></tr></form></table>");
    if ($_theme != "")
    _theme_after_content();
}
function _deny_access($message)
{
    global $_op, $_themedir, $_theme;
    if ($_theme != "") {
        include_once("$_themedir/theme.php");
        _theme_before_content();
    }
...
function _check_access($uid)
{
    ...
    $result = $db->sql_query($sql) or die("No se puede obtener los accesos del grupo");
...
if (isset($_COOKIE[_uid]))
{
    ...
    if ($db->sql_numrows($result))
        list($_default_module, $_theme) = $db->sql_fetchrow($result);
}
...
$__uid = $uid;
///////BODY///////
if ( (_check_access($uid)) &&  (_check_blocked($uid)) )
{   //si sí podemos ejecutar
    //registrar la fecha de la actividad
        if ($_COOKIE[_uid])
        {
            $db->sql_query("UPDATE users SET last_activity=NOW() WHERE uid='".$_COOKIE[_uid]."' LIMIT 1") or die("Error al actualizar tabla admins");
        }
    //checamos que el usuario ya haya seteado su password, si no, lo obligamos a hacerlo
    $sql = "SELECT uid FROM users WHERE uid='".$_COOKIE['_uid']."' AND password=PASSWORD(user) AND active=1";    

...
global $db,$_includesdir,$fecha_ini,$fecha_fin,$submit,$evento_id;
$tipo=$_REQUEST['tipo'];
...
if(isset($submit))
{
    ...
    switch($tipo)
    {
        case 1:
        {
            $sql_count="SELECT COUNT(*) as total_f FROM mfll_contactos_registro as a WHERE 1 ".$rango_fechas.";";
            $sql="SELECT b.fase_id as ids ,count(b.fase_id) as total FROM mfll_contactos_registro  as a, mfll_contactos_fases as b WHERE 1 ".$rango_fechas." AND a.contacto_id=b.contacto_id GROUP BY b.fase_id ORDER BY b.fase_id;";
        ...
        case 2:
        {
            $sql_count="SELECT COUNT(a.contacto_id) as total_f FROM mfll_contactos_registro as a WHERE 1 ".$rango_fechas.";";
            $sql="SELECT a.gid as ids ,count(a.gid) as total FROM mfll_contactos_registro as a WHERE 1 ".$rango_fechas." GROUP BY a.gid ORDER BY a.gid;";
...
if(isset($submit))
{
    ...
    switch($tipo)
    {
        case 1:
        {
            $sql_count="SELECT COUNT(*) as total_f FROM mfll_contactos_registro WHERE 1 ".$rango_fechas.";";
            $sql="SELECT b.fase_id as ids ,count(b.fase_id) as total FROM mfll_contactos_registro  as a, mfll_contactos_fases as b WHERE 1 ".$rango_fechas." AND a.contacto_id=b.contacto_id GROUP BY b.fase_id ORDER BY b.fase_id;";
            $titulo="Gráfica por Fases";
            $tmp_celda="Fases";
            break;
        ...
        {
            $sql_count="SELECT COUNT(*) as total_f FROM mfll_contactos_registro WHERE 1 ".$rango_fechas.";";
            $sql="SELECT gid as ids ,count(gid) as total FROM mfll_contactos_registro WHERE 1 ".$rango_fechas." GROUP BY gid ORDER BY gid;";
            $titulo="Gráfica por Concesionarias";
            $tmp_celda="Concesionarias";
        }
    }
    ...
    if($db->sql_numrows($res) > 0)
    {
        ...
        while(list($ids,$total) = $db->sql_fetchrow($res))
        {
            ...
            $array_url[]="index.php?_module=Reportes&_op=".$archivo_grafica."&tipo=".$tipo."&fecha_ini=".$fecha_ini."&fecha_fin=".$fecha_fin."&evento_id=".$evento_id."&id=".$ids;
            $promedio=0;
            if($total_bd != 0)
                $promedio=(($total/$total_bd)*100);
